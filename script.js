...
const REPO = "mansoorXcoder/academic-records";

/**
 * Show only these root-level folder names on the home screen.
 * Must match exactly (case-sensitive) what GitHub returns.
 * Set to null to show ALL root folders.
 * @type {string[] | null}
 */
...
async function goHome() {
  ...
  try {
    ...
    if (ALLOWED_FOLDERS && ALLOWED_FOLDERS.length) {
      folders = folders.filter(f =>
        ALLOWED_FOLDERS.some(a => a.toLowerCase() === f.name.toLowerCase())
      );
    }
